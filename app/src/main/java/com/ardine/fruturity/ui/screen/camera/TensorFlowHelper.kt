...
object TensorFLowHelper {
    ...
    fun classifyImage(image: Bitmap, callback : (@Composable (fruit : String) -> Unit)) {
        val model: ModelFp = ModelFp.newInstance(LocalContext.current)
        ...
        val outputs: ModelFp.Outputs = model.process(inputFeature0)
        ...
        val classes = arrayOf("matang", "busuk", "mentah")
